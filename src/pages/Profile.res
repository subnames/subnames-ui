...
let getNameExpiry = async (name: string) => {
  try {
    let expiry = await OnChainOperations.nameExpires(name)
    expiry
  } catch {
  | Exn.Error(e) => {
    ...
    React.useEffect(() => {
      let checkOwnership = async () => {
        if account.isConnected {
          try {
            let owner = await OnChainOperations.getTokenOwner(name)
